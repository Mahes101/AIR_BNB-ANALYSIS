...
def insights():
        ...
        ques = ['Question-1','Question-2','Question-3','Question-4','Question-5','Question-6','Question-7','Question-8','Question-9']
        ...
        if opt4 == 'Question-1':
                ...
                st.write(f"There are totally {how_many_host_available_for_365_days} host which has availability of 365 days")
                ...
                st.header("CHEAPEST AVG PRICE OF COUNTRY")     
                ...
                st.header("CHEAPEST PRICE OF ROOM TYPE")
                ...
                st.header("MINIMUM NIGHTS STAYED PROPERTY TYPE")
                ...
                sns.set_theme(style='white', font_scale=3)
                st.pyplot()
        elif    opt4 == 'Question-9':
                st.header("TOTAL LISTINGS BY ROOM TYPE ACROSS COUNTRIES ")
                df_subset = df1[['Room_type', 'Id', 'Country']]
                df_agg = df_subset.groupby(['Country', 'Room_type'],as_index=False).agg(total_ids=('Id', 'nunique')).sort_values(by = 'total_ids', ascending = False)
                fig = px.bar(
                df_agg[['Room_type', 'total_ids', 'Country']],
                x='Country',
                y='total_ids',
                color='Room_type',
                width=900,
                height=400,
                title='<em>Total Listings by Room Types Across Country</em>',
                template='plotly_dark',
                barmode = 'stack',
                labels={'total_ids': '<b>Total Listings</b>',
                        'Country': '<b>Country</b>',
                        'Room_type': '<b>Room Type</b>'
                        },
